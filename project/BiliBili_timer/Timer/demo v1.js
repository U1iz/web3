...
(function () {
    ...
    function userLeave() {
        ...
        sync(JSON.parse(localStorage.getItem(storageKeySet[1]))[window.currentTime[0]], temp_data[window.currentTime[0]], storageKeySet[1])
    ...
    window.addEventListener('storage', e => {
        if (e.key == storageKeySet[1] && e.url != location.href) {
            ...
            if (Object.keys(ns[window.currentTime[0]]).length != Object.keys(temp_data[window.currentTime[0]]).length) {
                sync(temp_data[window.currentTime[0]], ns[window.currentTime[0]], storageKeySet[1])
            }
        }
    })
