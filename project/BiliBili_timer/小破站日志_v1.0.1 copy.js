...
(function () {
    ...
    function save() {
        ...
        let local = JSON.parse(localStorage.getItem('bilibili_userLog'));
        console.log(local, b_log,window.currentDate);
    ...
    function get_project_info() {
        let info = {}
        {
            const vd = document.querySelector('.video-data-list');
            const viewer = document.querySelector('#arc_toolbar_report .toolbar-left')
            if (vd) {
                let temp = vd.innerText.split('\n')
                let temp2 = viewer = viewer.innerText.split('\n')
                info = {
                    view: temp[0],
                    dm: temp[1],
                    update: temp[2],
