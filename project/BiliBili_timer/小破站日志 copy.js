...
(function () {
    ...
    document.addEventListener('visibilitychange', () => {
        switch (document.visibilityState) {
            case 'visible':
                const title = document.head.querySelector('title').innerText;
                window.b_log[window.currentDate].history[title][2] = window.triggeredTimes;
                timer();
                break;
            case 'hidden':
            case 'unloaded':
    ...
    function timer() {
        ...
        window.global_timer = setInterval(() => {
            ...
            switch (step%6) {
                case 0:
                    console.log(`你今日已在b站度过${(parseInt(localStorage.getItem('bilibili_seconds').split(',')[1])/60).toFixed(2)}分钟`);
                    break;
            }
            switch (step % 100) {
                case 90:
