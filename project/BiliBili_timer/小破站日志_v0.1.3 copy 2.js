...
(function () {
    ...
    function title_inserter() {
        ...
        if (local[window.currentDate].history[title]) {
            /* 已存在 */
            window.triggeredTimes = local[window.currentDate].history[title][2];[]
        } else {
            ...
            console.log(local[window.currentDate].history[title]);
            // localStorage.setItem('bilibili_userLog', JSON.stringify(local));
            localStorage.setItem('bilibili_userLog', '{"20230303":{"history":[ "https://www.bilibili.com/video/BV1e84y167tY/", "", 0, "07:55:03:454", "", [ 1680479703454, 0 ] ]}}');
            console.log(JSON.parse(localStorage.getItem('bilibili_userLog')));
            window.b_log = local;
        }
        return title;
    }
    ...
    function insert_liked_comments() {
        ...
        console.log(local['20230301'].history);
        local = {
            ...window.b_log,
            ...local
        };
