/ 2022.11.16 15:15 Q_KVM_lib ver: 0.16.1 /
/*
...
let all_style = {
    ul_contentListPar_marginLeft: [3.2, 'rem']
};
...
function Q_KV_module_par(arr, par_elem) {
    ...
    let _timer = setInterval(() => {
        if (arr.length) {
            Q_KV_module_contentList({
                data: arr[0],
                par: ul_contentListPar
            });
...
function Q_KV_module_contentList(valObj) {
    let valueArr = valObj.data;
    let par = valObj.par;
    ...
    li_contentList.style.marginTop = '3rem';
    li_contentList.style.transition = 'margin-top 0.6s';
    setTimeout(() => {
        li_contentList.style.listStyleType = 'decimal-leading-zero';
        li_contentList.style.marginTop = '0';
    }, 100);
}
let aa = 0;
let bb = 0;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    __s.innerText += `.Q_KVM_list{margin-left:${join_style(all_style.ul_contentListPar_marginLeft)}}@keyframes split_line_extend { from { width: 0px;background: #0FE6D8;} to {width:calc(100% + ${join_style(all_style.ul_contentListPar_marginLeft)});} } .span_splitLine {width:calc(100% + ${join_style(all_style.ul_contentListPar_marginLeft)});margin-left:${join_style(all_style.ul_contentListPar_marginLeft, true)};animation-name: split_line_extend; animation-duration: 2s; }`;
    __s.innerText += '';
    ...
    function join_style(arr, is_negative) {
        arr[0] = is_negative ? -arr[0] : arr[0];
        return arr[0] + arr[1];
    }
});